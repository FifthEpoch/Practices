...
int main(int argc, char **argv) {
    ...
    if (argc != 5)
    {
        fprintf(stderr, "usage: %s <n> <p> <d> <g>\n", argv[0]);
        ...
        fprintf(stderr, "             <g> is how many generations does this game run for)\n");
        return 1;
    }
    ...
    if (density < 0 || density > 100) print_error("ERROR: density must be a value in 1 to 100");
    
    int max_gen = (int)strtol(argv[4], NULL, 10);
    if (density < 1) print_error("ERROR: game must go on for a positive, non-zero amount of generations ");
    ...
    for (int i = 0; i < (dim_w * dim_h); i++) {
        int random = rand() % 100;
        pImageData[i] = (random < density) ? 0 : 1;
    }
